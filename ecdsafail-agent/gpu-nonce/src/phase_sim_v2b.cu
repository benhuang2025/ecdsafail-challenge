...
extern "C" __global__ void phase_sim_v2b(
    ...
    int                   write_out)            // 1 = write validation outputs, 0 = bench only
{
    ...
    for (u64 oi = 0; oi < n_ops; oi++, p += 12) {
        unsigned char kind  = p[0];
        unsigned char zflag = p[1];
        int qc2 = *(const short*)(p + 2);
        int qc1 = *(const short*)(p + 4);
        int qt  = *(const short*)(p + 6);
        int st  = *(const short*)(p + 8);
        int sc  = *(const short*)(p + 10);
