...
// phase_sim_v2.cu (1 thread = full batch, write_out=1), compare to Rust golden.

use cudarc::driver::{CudaContext, LaunchConfig, PushKernelArg};
use cudarc::nvrtc::compile_ptx;
use std::time::Instant;

const SRC: &str = include_str!("../phase_sim_v2.cu");
...
fn main() {
    ...
    let ops_raw = std::fs::read(format!("{DUMP}/ops2.bin")).unwrap();
